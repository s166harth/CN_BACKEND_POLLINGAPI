...
router.delete('/:id/delete', async (req, res) => {
    try {
      const option = await Option.findOne({ optionid: req.params.id });
      if (!option) {
        return res.status(404).json({ message: 'Option not found' });
      }
      const question = await Question.findOne({ 'options.optionid': req.params.id });
      if (!question) {
  ...
  router.put('/:id/add_vote', async (req, res) => {
    try {
      ...
      res.status(200).json({ message: 'Vote added successfully!', option });
    } catch (error) {
      console.error(error);
